...
async function calcSupplierMonthRebate(supplierId, month) {
  ...
  for (const [category, purchaseAmount] of byCategory) {
    const rule = await getEffectiveRule(supplierId, category);
...
/**
 * 订单完成时的本单预估返点：按当月该供应商分品类累计额（含本单）实时定档，
 * 本单返点 = Σ(本单分品类金额 × 当月累计定档后的边际费率)
 * 返回 { month, preRebate, preRebateRate }
 */
async function estimateOrderRebate(order) {
  const month = monthKeyOf(order.receiveAt || new Date());
  const orders = await getMonthCompletedOrders(order.supplierId, month);
  // 聚合口径含本单（extraOrder），byCategory 为含本单后的月累计分品类额
...
async function settleMonth(month, opts = {}) {
  ...
  for (const [sid, orders] of bySupplier) {
    ...
    const supplier = await Supplier.findById(sid).select('name balance').lean();
    ...
    const run = async (session) => {
      ...
      let totalPurchase = 0;
      let totalRebate = 0;
      // 品类 → 结算综合费率（用于回写订单实际返点）
      const catRateMap = new Map();

      for (const [category, purchaseAmount] of byCategory) {
        const rule = await getEffectiveRule(sid, category);
        const r = calcRebate(rule, purchaseAmount);
        details.push({
          category,
          purchaseAmount,
      ...
      // 回写订单实际返点（按订单分品类金额 × 结算综合费率分摊）
      for (const o of orders) {
        const orderCats = perOrder.get(String(o._id)) || new Map();
        let orderRebate = 0;
        for (const [cat, amt] of orderCats) {
          const rate = catRateMap.has(cat) ? catRateMap.get(cat) : DEFAULT_REBATE_RATE;
...
async function getMonthOverview(month) {
  const suppliers = await Supplier.find().select('name').sort({ createdAt: 1 }).lean();
